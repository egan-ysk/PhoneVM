...
- Physical-device identifiers are used locally to target the selected device. Screenshots are copied to the macOS clipboard or saved as PNG files to the user's Desktop, depending on the selected action. PhoneVM does not upload screenshots; desktop files remain until the user removes them, and filenames contain no device name or identifier.
